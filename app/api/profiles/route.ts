...
export async function POST(request: Request) {
  ...
  if (resolvedUserId?.includes("@")) {
    try {
      ...
      // @ts-ignore
      const getUser = await supabaseAdmin.auth.admin.getUserByEmail(
        resolvedUserId,
      );
    ...
    try {
      ...
      if (cols.length === 0) {
        try {
          ...
          // eslint-disable-next-line no-console
          console.warn("professionals minimal insert failed:", e?.message ?? e);
        }
      } else {
        try {
      ...
      // eslint-disable-next-line no-console
      console.warn("professionals upsert skipped/failed:", professionalError?.message ?? professionalError);
    }
  }
  ...
  if (!professional) {
    try {
      ...
      if (!authUser) {
        ...
        if (found) {
          const metadata = found.user_metadata as Record<string, any> | undefined;
          const profObj = {
            id: found.id,
            user_id: found.id,
